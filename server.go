...
type serverConn struct {
	...
	// streams and active track the in-flight streams of this connection.
	// They are fields rather than locals of run so that package-private
	// test hooks can observe the server-side stream set and its cleanup.
	streams sync.Map // map[uint32]*streamHandler
	active  int32    // accessed atomically
