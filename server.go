...
type serverConn struct {
	...
	// streams holds the active stream handlers for this connection, keyed
	// by stream id, and active counts them. done is closed when run
	// returns. These are fields (rather than locals of run) so that tests
	// can observe the stream lifecycle and connection teardown
	// deterministically; access patterns are unchanged.
	streams sync.Map
	active  int32
	done    chan struct{}
}
